<?php
/**
 * File containing the AbstractNode class.
 *
...
abstract class Node implements VisitableInterface
{
     ...
     *
     * @param  AbstractNode $node The node that is to be added as incoming node.
     * @throws InvalidWorkflowException if the operation violates the constraints of the nodes involved.
     * @return AbstractNode
     */
    public function addInNode(Node $node)
    {
        // Check whether the node is already an incoming node of this node.
        if ( Util::findObject( $this->inNodes, $node ) === false )
        {
     ...
     *
     * @param  AbstractNode $node The node that is to be removed as incoming node.
     * @throws InvalidWorkflowException if the operation violates the constraints of the nodes involved.
     * @return boolean
     */
    public function removeInNode(Node $node)
    {
        $index = Util::findObject( $this->inNodes, $node );
     ...
     *
     * @param  AbstractNode $node The node that is to be added as outgoing node.
     * @throws InvalidWorkflowException if the operation violates the constraints of the nodes involved.
     * @return AbstractNode
     */
    public function addOutNode(Node $node)
    {
        // Check whether the other node is already an outgoing node of this node.
        if ( Util::findObject( $this->outNodes, $node ) === false )
        {
     ...
     *
     * @param  AbstractNode $node The node that is to be removed as outgoing node.
     * @throws InvalidWorkflowException if the operation violates the constraints of the nodes involved.
     * @return boolean
     */
    public function removeOutNode(Node $node)
    {
        $index = Util::findObject( $this->outNodes, $node );
     ...
     *
     * One of AbstractNode::WAITING_FOR_ACTIVATION or
     * AbstractNode::WAITING_FOR_EXECUTION.
     *
     ...
     * @param AbstractNode $activatedFrom
     * @param int $threadId
     * @ignore
     */
    public function activate(Execution $execution, Node $activatedFrom = null, $threadId = 0 )
    {
     ...
     */
    protected function activateNode(Execution $execution, Node $node )
    {
     ...
     *
     * Implementations of AbstractNode should reimplement this method.
     *
    ...
    {
        $type   = str_replace( 'Opensoft\\Workflow\\Nodes\\', '', get_class( $this ) );
        $max    = strlen( $type );
        $string = '';
